...
class DataHandler:
    ...
    def _crop_imgs(self, imgs, batch_size, flatness):
        ...
        for i, axis in enumerate(['x', 'y']):
            ...
            # print(imgs['lr'].shape[i])
            # print(self.patch_size['lr'])
            # print(batch_size)
            # input()
    ...
    def get_batch(self, batch_size, idx=None, flatness=0.0):
        ...
        # correct hr image dimension ----------------------------------------
        img['hr'] = self.correct_hr_dim(img['lr'], img['hr'], self.scale)
        # -------------------------------------------------------------------
    ...
    def correct_hr_dim(self, lr_img, hr_img, scale):
        """ Truncate/padd(zero padding) hr_img such that:
            (hl_img_h * scale,lr_img_w*scale) = (hr_img_h,hr_img_w)
        Arg:
        lr_img -- numpy array of lr image
        hr_img -- numpy array of hr image
        scale -- scaling factor(integer)

        returns:
        corrected_img/hr_img -- numpy array of truncated/padded/unchanged
            hr_img
        """
        # if dimension are already correct return same as it is.
        # print(lr_img.shape[0]*scale)
        # print(hr_img.shape[0])
        # print(lr_img.shape[1]*scale)
        # print(hr_img.shape[1])
        # input()
        # exit(0)
        
        if (lr_img.shape[0]*scale,
            lr_img.shape[1]*scale) == (hr_img.shape[0],
                                       hr_img.shape[1]):
            return hr_img
        else:
            ...
            if lr_img.ndim == 3:
                # color image
                new_img = np.zeros(
                    (lr_img.shape[0]*scale, lr_img.shape[1]*scale, 3))
                new_img[0:hr_img.shape[0],
                        0:hr_img.shape[1]] = hr_img[0:new_img.shape[0],
                                                    0:new_img.shape[1]]

            if lr_img.ndim == 2:
                # Gray scale image
                new_img = np.zeros(
                    (lr_img.shape[0]*scale, lr_img.shape[1]*scale))
                new_img[0:hr_img.shape[0],
                        0:hr_img.shape[1]] = hr_img[0:new_img.shape[0],
                                                    0:new_img.shape[1]]
